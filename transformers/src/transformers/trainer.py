...
class Trainer:
    ...
    def __init__(
        ...
        # NOTE model.is_parallelizable = False
        # "model_parallel" not in model
        if getattr(model, "is_parallelizable", False) and getattr(model, "model_parallel", False):
            self.is_model_parallel = True #
        else:
            self.is_model_parallel = False #HERE because model_parallel not in

        # "hf_device_map" not in model,  
        ...
        if self.args.use_liger_kernel: # NOTE =True
        ...
        #    'min_num_params':0, 'xla':False, 'xla_fsdp_v2':False,
        #    'xla_fsdp_grad_ckpt': False}
        self.is_fsdp_xla_enabled = args.fsdp_config["xla"]

        # args.fsdp=[<FSDPOption.FULL_SHARD: 'full_shard'>, 
        #            <FSDPOption.AUTO_WRAP: 'auto_wrap'>]
        if len(args.fsdp) > 0:
        ...
        ) # NOTE SKIP this since we have data_collator
        ...
        if self.is_model_parallel:
            self.args._n_gpu = 1 #NOT HERE but it's already set 1
        ...
        self.compute_metrics = compute_metrics #NOTE =None
        self.preprocess_logits_for_metrics = preprocess_logits_for_metrics #=None
        self.optimizer, self.lr_scheduler = optimizers #(None, None)
    ...
    def train(
        ...
        **kwargs,
        ...
        # port = 4444+dist.get_rank()
        # print(f"Process {port} waiting for debugger on port {port}")
        # rpdb.set_trace(port=port)#fp16_backend=auto
        if ( # NOTE SET device for training just in case do_train is forgotten to set=True
            (args.fp16_full_eval or args.bf16_full_eval)#False or False
            and not args.do_train # do_train=False
            and not self.is_model_parallel#is_model_parallel=False
            and self.model_init is None #True
        ):
            self._move_model_to_device(self.model, args.device)

        if "model_path" in kwargs: #kwargs={}
        ...
        self._train_batch_size = self.args.train_batch_size#=1

        # Model re-init
        model_reloaded = False
        # NOTE this part below is not called
        if self.model_init is not None:#self.model_init=None
        ...
        if model_reloaded: # NOTE not called
    ...
    def _inner_training_loop(
        ...
        num_train_tokens = None # NO NEED TO COMPTUTE THIS FOR MY CASE
        ...
        # NOTE is_fsdp2=False, is_fsdp_enabled=True, 
        # self.accelerator.state.fsdp_plugin = FullyShardedDataParallelPlugin(fsdp_version=1, sharding_strategy=... )
        ...
        self.state.compute_steps(args, max_steps) #max_steps=1250
        ...
        if delay_optimizer_creation: # NOTE True
            if use_accelerator_prepare: # NOTE True
                ...
                if self.accelerator.mixed_precision != "fp8": # self.accelerator.mixed_precision='bf16'
                    self.model = self.accelerator.prepare(self.model) #NOTE SJ: this part will hung until all processes pass
            self.create_optimizer_and_scheduler(num_training_steps=max_steps) #max_steps=1250
            ...
            for _ in range(total_updates):
                ...
                for i, inputs in enumerate(batch_samples):
                    ...
                    with context():
                        ...
                        # tr_loss_step=0.5196, num_items_in_batch=21004
                        # import rpdb
    ...
    def compute_loss(
        ...
        # NOTE SJ: check if "attention_mask" and "position_ids" are already here
        # import rpdb
        # port = 4444+dist.get_rank()
        # print(f"Process {port} waiting for debugger on port {port}")
        # rpdb.set_trace(port=port)
        # NOTE inputs.keys()=['input_ids','attention_mask','labels','num_items_in_batch']
        # NO position_ids!
    ...
    def create_accelerator_and_postprocess(self):
        ...
        print(f"os-local-rank={local_rank}")
        # {'deepspeed_plugin': None, 'dataloader_config': Da}
        ...
        print(f"Local process index: {self.accelerator.local_process_index}")
        ...
        # Distributed environment: DistributedType.FSDP Backend:nccl
        # num processes: 8
        # process index: 0
    ...
    def get_batch_samples(
        ...
        # import ipdb; ipdb.set_trace() # all_processes_max_num_per_device_global_batch_size=8, nonpad_this_global_ki=8
        if num_items_in_batch is not None:
            if self.args.average_tokens_across_devices: # NOTE =False why????
    ...
    def set_initial_training_values(
        ...
        # NOTE SJ: no longer compute this because 
        # we pre-specify the number of updates per epoch now
        if len_dataloader is not None:
        ...
        if len_dataloader:
            ...
            else: # NOTE SJ: all of these will be specified.
