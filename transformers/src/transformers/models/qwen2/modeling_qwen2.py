...
class Qwen2Model(Qwen2PreTrainedModel):
    ...
    def forward(
        ...
        **flash_attn_kwargs: Unpack[FlashAttentionKwargs],
        ...
        # NOTE cache_position=None, past_key_values=None
        if cache_position is None:
            past_seen_tokens = past_key_values.get_seq_length() if past_key_values is not None else 0
            cache_position = torch.arange( #past_seen_tokens=0
                past_seen_tokens, past_seen_tokens + inputs_embeds.shape[1], device=inputs_embeds.device
            ) # NOTE inptus_embeds.shape=[1,10290,5120]
            # this will create ``position_ids'' for all, so it's okay not to have ``position_ids=None'' passed in

        # NOTE SJ position_ids=None for s1 codebase
        # import torch.distributed as dist
        # import rpdb
        # port = 4444+dist.get_rank()
        # print(f"Process {port} waiting for debugger on port {port}")
        # rpdb.set_trace(port=port)
        # NOTE cache_position.shape=torch.Size([10290])
        # NOTE input_embeds.shape=[1,10290,5120]
        # position_ids=None 
        if position_ids is None:
            position_ids = cache_position.unsqueeze(0)
        # position_ids.shape=[1,10290]
