...
import wandb
import numpy as np, torch, wandb

def _pop_histograms_from_logs(logs, keys, num_bins=64, also_normalized=True, name_map=None):
    out = {}
    for k in list(keys):
        if k not in logs:
            continue
        ...
        name_hist, name_norm = (f"{k}_hist", f"{k}_hist_norm")
        if name_map and k in name_map:
            name_hist, name_norm = name_map[k]

        out[name_hist] = wandb.Histogram(arr, num_bins=num_bins)
        if also_normalized:
            s = arr.sum()
            norm = arr / s if s > 0 else arr
            out[name_norm] = wandb.Histogram(norm, num_bins=num_bins)
    return out


def pop_bincount_histograms_from_logs(logs, *, keys, name_map=None):
    """
    Treat logs[key] as *bin counts per ID*, not raw samples.
    Replaces each key with a wandb.Histogram(np_histogram=(hist, edges)).
    """
    out = {}
    for k in list(logs.keys()):
        if k not in keys:
            continue
        v = logs.pop(k)
        arr = v.detach().flatten().cpu().numpy() if torch.is_tensor(v) else np.asarray(v).reshape(-1)
        if arr.size == 0:
            continue
        hist = arr.astype(np.float64)
        # bin edges: [0,1,2,...,N] → N bins aligned to integer IDs
        edges = np.arange(hist.size + 1, dtype=np.float64)

        name = name_map.get(k, k) if name_map else k
        out[name] = wandb.Histogram(np_histogram=(hist, edges))
    return out


# def _pop_histograms_from_logs(
#     logs: Dict,
#     *,
#     keys: Iterable[str],
#     num_bins: int = 64,
#     also_normalized: bool = True,
#     name_map: Dict[str, Tuple[str, str]] | None = None,
# ) -> Dict[str, wandb.Histogram]:
#     """
#     For each `key` in `keys`, pop logs[key] (list[int] / np / tensor),
#     and return {<new_name>: wandb.Histogram(...)} to merge into your log dict.
#     Creates a normalized variant as <name>_norm (or from name_map if provided).
#     """
#     out: Dict[str, wandb.Histogram] = {}
#     for k in list(logs.keys()):
#         if k not in keys:
#             continue
#         v = logs.pop(k)
...
#             arr = v.detach().flatten().cpu().numpy()
#         else:
...
#         if also_normalized:
#             denom = arr.sum()
#             norm = (arr / denom) if denom > 0 else arr
#             out[name_norm] = wandb.Histogram(norm, num_bins=num_bins)
#     return out
...
#     logs: Dict,
#     *,
#     keys: Iterable[str] = ("row_usage_counts",),  # keys to treat as histograms
#     num_bins: int = 64,
#     also_normalized: bool = True,
#     # optional renames; defaults produce "<key>_hist" and "<key>_hist_norm"
#     name_map: Dict[str, Tuple[str, str]] | None = None,
# ) -> Dict[str, wandb.Histogram]:
#     """
#     Finds specified keys in `logs`, converts their values to wandb.Histogram,
#     and pops them out of `logs`. Returns a dict of {new_key: Histogram}.
#     Accepts torch.Tensor (any device), numpy.ndarray, or list-like 1D data.
#     """
#     out: Dict[str, wandb.Histogram] = {}
#     for k in list(logs.keys()):
#         if k not in keys:
...
#         out[name_hist] = wandb.Histogram(arr, num_bins=num_bins)
# 
#         # normalized histogram (if requested)
#         if also_normalized:
#             denom = arr.sum()
#             if denom > 0:
#                 out[name_norm] = wandb.Histogram(arr / denom, num_bins=num_bins)
#             else:
#                 # empty/zero counts → still log something (all zeros)
#                 out[name_norm] = wandb.Histogram(arr, num_bins=num_bins)
#     return out
...
class WandbCallback(TrainerCallback):
    ...
    def on_log(self, args, state, control, model=None, logs=None, **kwargs):
        ...
        if state.is_world_process_zero:
            ...
            # hist_keys = [k for k in list(non_scalar_logs.keys())
            #      if k == "row_usage_counts"
            #      or k.startswith("rowset_counts_C")
            #      or k.startswith("config_usage_counts_C")]

            # name_map = {k: (k, None) for k in hist_keys}
            # hist_logs = _pop_histograms_from_logs(
            #     non_scalar_logs,
            #     keys=hist_keys,    # add more keys if you like
            #     num_bins=64, # NOTE change this later
            #     also_normalized=False,
            #     name_map=name_map,
            # )
            # non_scalar_logs.update(hist_logs)

            hist_keys = [k for k in list(non_scalar_logs)
                         if k in ("row_usage_counts", "row_token_counts", "row_tokens_per_use")
                         or k.startswith("rowset_counts_C")
                         or k.startswith("config_usage_counts_C")]
            
            # import rpdb
            # port = 4444+args.process_index
            # print(f"Process {args.process_index} waiting for debugger on port {port}")
            # rpdb.set_trace(port=port)
            # overwrite SAME keys so existing panels work
            name_map = {k: k for k in hist_keys}
            # hist_logs = pop_bincount_histograms_from_logs(non_scalar_logs, keys=hist_keys, name_map=name_map)
            # non_scalar_logs.update(hist_logs)
            media_logs = pop_bincount_histograms_from_logs(non_scalar_logs, keys=hist_keys, name_map=name_map)


            if state.global_step % 1 != 0:
                for hk in list(non_scalar_logs.keys()):
                    if hk.endswith("_hist") or hk.endswith("_hist_norm"):
                        non_scalar_logs.pop(hk, None)
            non_scalar_logs = rewrite_logs(non_scalar_logs)
            self._wandb.log(media_logs, step=state.global_step, commit=False)
            self._wandb.log({**non_scalar_logs, "train/global_step": state.global_step})
